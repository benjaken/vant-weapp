    ...
    methods: {
        ...
        loadImage(src) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                const { width, height, path } = yield wx.getImageInfo({
                    src: src || image,
                });
                ...
                const qualityWidth = this.conf.DRAW_IMAGE_W > this.conf.MAX_QW
                    ? this.conf.MAX_QW
        ...
        getImageInfo() {
            ...
            ctx.draw(true, () => __awaiter(this, void 0, void 0, function* () {
        ...
        initPosition() {
            ...
            const left = 0, right = 0, top = 0, bottom = 0;
            if (cutRatio === 0 && this.conf.IMG_RATIO >= 1) {
                return { left, right, top, bottom };
            }
            if (this.conf.IMG_RATIO >= 1) {
                if (this.conf.IMG_RATIO >= cutRatio) {
                    let leftRight = Math.ceil((cropperWidth - this.conf.CROPPER_HEIGHT * cutRatio) / 2);
                    return {
                        left: leftRight,
                        right: leftRight,
                        top: 0,
                ...
                const bottomTop = Math.ceil((this.conf.CROPPER_HEIGHT - cropperWidth / cutRatio) / 2);
                return {
                    left: 0,
                    right: 0,
                    top: bottomTop,
            ...
            if (cutRatio === 0)
                return { left, right, top, bottom };
            if (this.conf.IMG_RATIO >= cutRatio) {
                const leftRight = Math.ceil((this.conf.CROPPER_WIDTH - this.conf.CROPPER_HEIGHT * cutRatio) / 2);
                return {
                    left: leftRight,
                    right: leftRight,
                    top: 0,
            ...
            const bottomTop = Math.ceil((this.conf.CROPPER_HEIGHT - this.conf.CROPPER_WIDTH / cutRatio) / 2);
            return {
                left: 0,
                right: 0,
                top: bottomTop,
        ...
        sideDragMoveConst(e, type) {
            ...
            switch (type) {
                case 'top':
                    ...
                    top = Math.ceil(top >= this.drag.SPACE_TOP_POSITION
                        ? this.drag.SPACE_TOP_POSITION
                        : top);
