// @ts-ignore
"use strict";
function t(t) { for (var i = Object.create(null), e = t.split(","), s = e.length; s--;)
    i[e[s]] = !0; return i; }
function i(t, i) { for (var e = t.indexOf("&"); -1 !== e;) {
    var s = t.indexOf(";", e + 3), a = void 0;
    if (-1 === s)
        break;
...
s.prototype.parse = function (t) { for (var i = this.plugins.length; i--;)
    ...
    if (!this.hook(i) || n.ignoreTags[i.name])
        return void ("base" !== i.name || this.options.domain ? "source" === i.name && s && ("video" === s.name || "audio" === s.name) && e.src && s.src.push(e.src) : this.options.domain = e.href);
    ...
    if ("img" === i.name) {
        if (e.src && (e.src.includes("webp") && (i.webp = "T"), e.src.includes("data:") && !e["original-src"] && (e.ignore = "T"), !e.ignore || i.webp || e.src.includes("cloud://"))) {
            for (var p = this.stack.length; p--;) {
                var u = this.stack[p];
                "a" === u.name && (i.a = u.attrs), "table" !== u.name || i.webp || e.src.includes("cloud://") || (!c.display || c.display.includes("inline") ? i.t = "inline-block" : i.t = c.display, c.display = void 0);
                var g = u.attrs.style || "";
                if (!g.includes("flex:") || g.includes("flex:0") || g.includes("flex: 0") || c.width && !(parseInt(c.width) > 100))
                    if (g.includes("flex") && "100%" === c.width)
                        ...
                        g.includes("inline-block") && (c.width && "%" === c.width[c.width.length - 1] ? (u.attrs.style += ";max-width:" + c.width, c.width = "") : u.attrs.style += ";max-width:100%");
                else {
            ...
            if (this.imgList.includes(y)) {
                ...
                if (-1 !== x) {
                    ...
                    for (var b = y.substr(0, x); x < y.length && "/" !== y[x]; x++)
                        b += Math.random() > .5 ? y[x].toUpperCase() : y[x];
                    b += y.substr(x), y = b;
                }
            }
        ...
        return a.push(i), this.stack.push(i), void this.popNode();
    for (var w in c)
        c[w] && (e.style += ";".concat(w, ":").concat(c[w].replace(" !important", "")));
    e.style = e.style.substr(1) || void 0;
}
else
    ("pre" === i.name || (e.style || "").includes("white-space") && e.style.includes("pre")) && 2 !== this.pre && (this.pre = i.pre = 1), i.children = [], this.stack.push(i); a.push(i); }, s.prototype.onCloseTag = function (t) { t = this.xml ? t : t.toLowerCase(); var i; for (i = this.stack.length; i-- && this.stack[i].name !== t;)
    ; if (-1 !== i)
...
} }, s.prototype.popNode = function () { var t = this.stack.pop(), i = t.attrs, s = t.children, a = this.stack[this.stack.length - 1], r = a ? a.children : this.nodes; if (!this.hook(t) || n.ignoreTags[t.name])
    return "title" === t.name && s.length && "text" === s[0].type && this.options.setTitle && wx.setNavigationBarTitle({ title: s[0].text }), void r.pop(); if (t.pre && 2 !== this.pre) {
    this.pre = t.pre = void 0;
    for (var o = this.stack.length; o--;)
        this.stack[o].pre && (this.pre = 1);
} if ("svg" === t.name) {
    if (this.xml > 1)
        return void this.xml--;
    var l = "", c = i.style;
    return i.style = "", i.xmlns = "http://www.w3.org/2000/svg", function t(i) { if ("text" === i.type)
        return void (l += i.text); var e = n.svgDict[i.name] || i.name; l += "<" + e; for (var s in i.attrs) {
        var a = i.attrs[s];
        ...
        l += "/>"; }(t), t.name = "img", t.attrs = { src: "data:image/svg+xml;utf8," + l.replace(/#/g, "%23"), style: c, ignore: "T" }, t.children = void 0, void (this.xml = !1);
} var d = {}; if (i.align && ("table" === t.name ? "center" === i.align ? d["margin-inline-start"] = d["margin-inline-end"] = "auto" : d.float = i.align : d["text-align"] = i.align, i.align = void 0), i.dir && (d.direction = i.dir, i.dir = void 0), "font" === t.name && (i.color && (d.color = i.color, i.color = void 0), i.face && (d["font-family"] = i.face, i.face = void 0), i.size)) {
...
} if ((i.class || "").includes("align-center") && (d["text-align"] = "center"), Object.assign(d, this.parseStyle(t)), "table" !== t.name && parseInt(d.width) > h && (d["max-width"] = "100%", d["box-sizing"] = "border-box"), n.blockTags[t.name])
    t.name = "div";
else if (n.trustTags[t.name] || this.xml)
    if ("a" === t.name || "ad" === t.name)
        this.expose();
    else if ("video" === t.name || "audio" === t.name)
        (d.height || "").includes("auto") && (d.height = void 0), t.children = void 0;
    else if ("ul" !== t.name && "ol" !== t.name || !t.c) {
        if ("table" === t.name) {
            var u = parseFloat(i.cellpadding), g = parseFloat(i.cellspacing), f = parseFloat(i.border), m = d["border-color"], v = d["border-style"];
            if (t.c && (isNaN(u) && (u = 2), isNaN(g) && (g = 2)), f && (i.style += ";border:".concat(f, "px ").concat(v || "solid", " ").concat(m || "gray")), t.flag && t.c) {
                ...
                var y = [], x = [], b = [], w = {};
                !function t(i) { for (var e = 0; e < i.length; e++)
                    "tr" === i[e].name ? x.push(i[e]) : t(i[e].children || []); }(s);
                    ...
                    for (var N = 1, T = 0; T < x[k - 1].children.length; T++) {
                        ...
                        if ("td" === O.name || "th" === O.name) {
                            for (; w[k + "." + N];)
                                N++;
                            O.c = 1;
                            var C = O.attrs.style || "", S = C.indexOf("width") ? C.indexOf(";width") : 0;
                            if (-1 !== S) {
                                var I = C.indexOf(";", S + 6);
                            ...
                            if (C = (f ? ";border:".concat(f, "px ").concat(v || "solid", " ").concat(m || "gray") + (g ? "" : ";border-right:0;border-bottom:0") : "") + (u ? ";padding:".concat(u, "px") : "") + ";" + C, O.attrs.colspan && (C += ";grid-column-start:".concat(N, ";grid-column-end:").concat(N + parseInt(O.attrs.colspan)), O.attrs.rowspan || (C += ";grid-row-start:".concat(k, ";grid-row-end:").concat(k + 1)), N += parseInt(O.attrs.colspan) - 1), O.attrs.rowspan) {
                                ...
                                for (var L = 1; L < O.attrs.rowspan; L++)
                                    for (var z = 0; z < (O.attrs.colspan || 1); z++)
                                        w[k + L + "." + (N - z)] = 1;
                            }
                ...
                t.c && (d.display = "table"), isNaN(g) || (d["border-spacing"] = g + "px"), (f || u || t.c) && function i(e) { for (var s = 0; s < e.length; s++) {
                    var a = e[s];
            ...
            if ("ruby" === t.name) {
                ...
                for (var V = 0; V < s.length - 1; V++)
                    "text" === s[V].type && "rt" === s[V + 1].name && (s[V] = { name: "span", attrs: { style: "display:inline-block;text-align:center" }, children: [{ name: "div", attrs: { style: "font-size:50%;" + (s[V + 1].attrs.style || "") }, children: s[V + 1].children }, s[V]] }, s.splice(V + 1, 1));
            }
        }
        else
    ...
    t.name = "span"; if ((d.display || "").includes("flex") && !t.c)
    for (var Z = s.length; Z--;) {
        var _ = s[Z];
        _.f && (_.attrs.style = (_.attrs.style || "") + _.f, _.f = void 0);
    } var G = a && ((a.attrs.style || "").includes("flex") || (a.attrs.style || "").includes("grid")) && !t.c && !(d.display || "").includes("inline"); G && (t.f = ";max-width:100%"), s.length >= 50 && t.c && !(d.display || "").includes("flex") && e(s); for (var M in d)
    if (d[M]) {
    ...
    } i.style = i.style.substr(1) || void 0; }, s.prototype.onText = function (t) { if (!this.pre) {
    for (var e, s = "", a = 0, n = t.length; a < n; a++)
        c[t[a]] ? (" " !== s[s.length - 1] && (s += " "), "\n" !== t[a] || e || (e = !0)) : s += t[a];
    if (" " === s && e)
        return;
    t = s;
} var r = Object.create(null); if (r.type = "text", r.text = i(t), this.hook(r)) {
...
} }, a.prototype.parse = function (t) { this.content = t || "", this.i = 0, this.start = 0, this.state = this.text; for (var i = this.content.length; -1 !== this.i && this.i < i;)
    this.state(); }, a.prototype.checkClose = function (t) { var i = "/" === this.content[this.i]; return !!(">" === this.content[this.i] || i && ">" === this.content[this.i + 1]) && (t && this.handler[t](this.content.substring(this.start, this.i)), this.i += i ? 2 : 1, this.start = this.i, this.handler.onOpenTag(i), "script" === this.handler.tagName ? (this.i = this.content.indexOf("</", this.i), -1 !== this.i && (this.i += 2, this.start = this.i), this.state = this.endTag) : this.state = this.text, !0); }, a.prototype.text = function () { if (this.i = this.content.indexOf("<", this.i), -1 === this.i)
    return void (this.start < this.content.length && this.handler.onText(this.content.substring(this.start, this.content.length))); var t = this.content[this.i + 1]; if (t >= "a" && t <= "z" || t >= "A" && t <= "Z")
    ...
    this.i++; }, a.prototype.tagName = function () { if (c[this.content[this.i]]) {
    ...
    this.checkClose("onTagName") || this.i++; }, a.prototype.attrName = function () { var t = this.content[this.i]; if (c[t] || "=" === t) {
    ...
    for (var i = "=" === t, e = this.content.length; ++this.i < e;)
        if (t = this.content[this.i], !c[t]) {
            if (this.checkClose())
                return;
            if (i)
                return this.start = this.i, void (this.state = this.attrVal);
    ...
    this.checkClose("onAttrName") || this.i++; }, a.prototype.attrVal = function () { var t = this.content[this.i], i = this.content.length; if ('"' === t || "'" === t) {
    ...
    for (; this.i < i; this.i++) {
        ...
        if (this.checkClose("onAttrVal"))
            return;
    } for (; c[this.content[++this.i]];)
    ; this.i < i && !this.checkClose() && (this.start = this.i, this.state = this.attrName); }, a.prototype.endTag = function () { var t = this.content[this.i]; if (c[t] || ">" === t || "/" === t) {
    if (this.handler.onCloseTag(this.content.substring(this.start, this.i)), ">" !== t && (this.i = this.content.indexOf(">", this.i), -1 === this.i))
        return;
    this.start = ++this.i, this.state = this.text;
}
else
    this.i++; }, module.exports = s;
