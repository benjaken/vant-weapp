(function(global, factory) {
  ...
  const [name$1t, bem$1o, t$j] = createNamespace("picker");
  ...
  var stdin_default$1x = vue.defineComponent({
    ...
    setup(props, {
      ...
      const renderNav = () => children.map((item, index) => vue.createVNode(stdin_default$1A, vue.mergeProps({
        "key": item.id,
        "id": `${id}-${index}`,
  ...
  function runRuleValidator(value, name2, rule) {
    return new Promise((resolve) => {
      const returnVal = rule.validator(value, name2, rule);
      if (isPromise(returnVal)) {
    ...
    setup(props, {
      ...
      const renderTipContent = () => {
        ...
        if (type === "password" && passwordTip.length > 0) {
          return passwordTip.map((item) => vue.createVNode("div", {
            "class": bem$1b("tip-item")
          }, [vue.createVNode(Icon, {
            "class": bem$1b("tip-item-icon"),
            "name": validateValue(getModelValue(), item.reg) ? "success" : "cross",
            "color": validateValue(getModelValue(), item.reg) ? "#07c160" : "#D92324"
          }, null), vue.createVNode("span", null, [item.text])]));
        }
        return vue.createVNode("span", null, [formatValue2(getModelValue(), FieldTipType || "mobile")]);
      };
      const renderTip = () => {
        const {
          type,
        ...
        if (showTip.value && type !== "textarea") {
          return vue.createVNode("div", {
            "class": bem$1b("tip", [inputAlign]),
            "style": tipStyle.value
          }, [vue.createVNode("div", {
            "class": bem$1b("tip-inner", {
              password: passwordTip.length > 0
            })
          }, [renderTipContent(), tipUnit ? vue.createVNode("span", {
            "class": bem$1b("tip-unit")
          }, [tipUnit]) : null])]);
        }
      };
      const renderLabel = () => {
  ...
  const [name$_, bem$Z, t$e] = createNamespace("cascader");
  const cascaderProps = {
    title: String,
    options: makeArrayProp(),
    closeable: truthProp,
  ...
  const [name$Y, bem$X] = createNamespace("check-button");
  ...
  var stdin_default$15 = vue.defineComponent({
    ...
    setup(props, {
      ...
      const renderButtons = () => {
        ...
        return options.map((item, index) => vue.createVNode("div", {
          "class": bem$X("item", {
  ...
  const CheckButton = withInstall(stdin_default$15);
  ...
  var stdin_default$Q = vue.defineComponent({
    ...
    setup(props, {
      ...
      const renderMessage = (hasTitle) => {
        ...
        return vue.createVNode("div", {
          "class": classNames
        }, [content, theme === "input" ? vue.createVNode("input", {
          "class": bem$H("input"),
          "placeholder": inputPlaceholder,
          "onBlur": validateInput,
          "onInput": onInput
        }, null) : "", inputError.value ? vue.createVNode("div", {
          "class": bem$H("error-message")
        }, [inputErrorMessage]) : ""]);
      };
      const renderContent = () => {
        if (slots.default) {
  ...
  var stdin_default$k = vue.defineComponent({
    ...
    setup(props, {
      ...
      const renderRows = () => Array(+props.row).fill("").map((_, i) => vue.createVNode(stdin_default$l, {
        "key": i,
        "round": props.round,
    ...
    setup(props, {
      ...
      const renderWatermark = () => {
        const svgInner = () => {
          ...
          }, [props.content ? vue.createVNode("span", {
            "style": {
              color: props.textColor
            }
  ...
  exports2.checkButtonProps = checkButtonProps;
