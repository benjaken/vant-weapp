...
(0, component_1.VantComponent)({
    ...
    mounted: function () {
        var _this = this;
        var _a = this.properties, steps = _a.steps, max = _a.max, min = _a.min, showTip = _a.showTip;
        ...
        if (showTip) {
            this.setData({
                displaySteps: Array.from({
                    ...
                    return _this.data.unlimited && index === steps ? '不限' : "".concat(min + step * index).concat(_this.data.unit || '');
                })
            });
        }
    },
    methods: {
        ...
        updateValue: function (value, end, drag) {
            ...
            var height = this.data.barHeight + (this.properties.showTip ? 73 : 0);
            this.setData({
                wrapperStyle: "\n          background: ".concat(this.data.inactiveColor || '', ";\n          ").concat(vertical ? 'width' : 'height', ": ").concat((0, utils_1.addUnit)(vertical ? this.data.barHeight : height) || '', ";\n        "),
